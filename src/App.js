...
function handleAddWatched(movie){
  setWatched((Wached) => [...Wached, movie]);
  ...
  return (
    <>
      ...
      <Main>
        ...
        /> */}
        <Box>
          {/* { isLoading ? <Loder /> : <MovieList movies={movies} />} */}
          {isLoading && <Loder />}
...
function MovieDetails({selectedId, onCloseMovie, onAddWached, watched}){
  ...
  useEffect( function() {
    // how many times we rated the movie
  ...
  const watchedUserRatting = watched.find((movie) => movie.imdbID === selectedId)?.userRating
  ...
  <div className="details">
    ...
    <section>
      ...
      :
      ( 
        <p>You rated this movie {watchedUserRatting} <span>⭐</span></p>
      )}
      </div>
      <p>
        <em>{plot}</em>
...
function WachedMoviesList({watched, onDeleteWatched}){
  return (
    <ul className="list">
        {watched.map((movie) => (
          <WachedMovie 
